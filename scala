...
Spark очень умный и делает почти на всем оптимизацию. к примеру если мы имеем action take(10), он не будет считать всю коллекцию, а затем брать 10 элементов, он посчитает только 10 элементов, а затем вернет их.
...
Spark app initialization: создается SparkConxtext, SparkConxtext коннектится к cluster manager, назначет workers, driver program отправляет код to executors, SparkConxtext отправляет таски executors.
...
Сам по себе Partition нужно знать в тех случаях, когда мы хотим получить некий перфоманс и уменьшить shuffle.
...
Обязательно следует помнить то, что есть некоторые операции которые ведут и к сохранению partition и к ее удалению.
