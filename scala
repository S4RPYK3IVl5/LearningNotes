...
Class
Так же как и в java являются шаблонами для методов и полей. Есть еще один вид такой как case class, так же нет как такого static class, но зато есть object.

Traits.
Являются интересами в Scala такими же как и в Java, могут иметь реализацию методов и свои поля.

Packages.
То же самое что и пакеты в Java. Могут быть импортированы из пакета целые классы, так же можно импортировать все методы из специфического объекта.
...
В Scala обычно используется СВV. Так как правило все аргументы вычисляются.
Рекурсиные выражения в Scala должны иметь явное возвращаемое значение.

Примитивные типы такие же как и в джава. Только большой буквой в начале. 

Существует такие выражения как if-else (в Scala оно представленно в виде выражения, а не утверждения). 
...
Class - создает новый тип в Scala, со своими специфичными свойствами. C помощью constructor мы можем создать элемент определенного типа (oblect) путем написания new Claszz(...).
...
scala.Null - сабтип класса scala.AnyRef (по сути просто null is instance of scala.Null). Совместим со всеми классами которые насчледуебтся от класса AnyRef, не совместим с классами от AnyVal.
scala.Nothing - это subtype scala.Any который обозначает отсутсвие инстансы любого объекта. Он применяется в методах, которые должны выбрасывать Exception. Так же представляет тип Nil. Cоздан для того чтобы удовлетворить JVM. Представляет пустую коллекцию.
...
List:
 ...
 Фундаментальная структура данных которая проедставленна в виде x1,x2,...,xn : List(x1,x2,...,xn). 
...
Vector:
 ...
 Очень хорош для bulk операций, трибующих прохождения по последовательности, к примеру map filter или fold.
 Способ хранения здесь так же выигрыавет, так как данные хранятся рядом в массиве.
...
Seq:
 ...
 Методы: 
  ...
  flatMap - предоставляет collection - value функции ко всем элементам в коллекции, и конкатенирует результат, создавая новую, по сути коллекцию. Аналогично: map.flatten
...
Arrays and String - имеют все те же методы что и Seq. Они неявно конвертируются в коллекции где нужно, они не плодклассы Seq, так как они берутся из java.

Range:
 Представляет последовательность равномерно распределенных чисел.
 Насладится от Seq.
 Представляет собой объект, имеющий всего 3 поля: lower bound, upper bound, step. Может быть создан с помощью: 1 until 5, 1 to 5, 1to 10 by 3, 6 to 1 by -2.
...
For-Expression - синтаксический сахар для foreach, map, flatMap, filter or withFilter операций на тех класса которые их реализуют.
 ...
 По существу эквиывалентен запросам в RDBMS.
 Работает таким образом что постепенно уменьшается до простейшего вида for(x <- ...) yield  ex.
 For expression совет работать не только с коллекциями, его работа базируется на присутствии в классе методов map, flatMap, withFilter.
...
Map:
 ...
 withDefaultValue - превращает маму в маму с дефолтным значением, если элемент не найден, что делает ее total function, a не partial function.
...
Так же модно написать таким образом: map {case (...) => ...} где преобразования будут применяться к каждому элементу подходящему по шаблону. Само по себе выражение не типизировано, следовательно нужно предписать ожидаемый параметр.
...
Здесь выступает та проблема, что у нас каждый раз перечисляется хвост при его вызове. Но это можно избежать, если использовать lazy evaluation [lazy val]. Scala по дефолту использует Strict evaluation. Фактически м можем быть носителями бесконечной последовательности.
...
Implict аргумент у функции может быть только один и он должен быть последним.
При поиске implict кандидата, компилятор ищет все параметры параметризованные тем же типом, затем он ищет марку implict в объявлении и последнее, взятый кандидат должен быть виден с точки вызова. 
def, val, lazy val, or object definition могут быть помеченны как implicit. Implicit парматр так же может принимать implicit и обычные параметры.
...
Есть одна огромная проблема при работе с mutable переменными, к примеру, a = 2; b = 2 * a; a = 3 => b - не изменится, из этого следует, что если что-то меняется, нас надо помнят все остальное в ручную, с другой стороны в immutable такой фигни нет.
...
Так же важно понимать, что когда меняется како-лито сигнал, нужно так же менять и другие сигналы, которые зависят от него.
...
Synchronized - гарантирует то что код внутри этого кода никогда не будет выполняться двумя потоками одновременно. Другими словами позволяет достигнуть atomicity. Со стороны JVM это реализуется подтем хранения monitor ни каждом объекте.
...
Есть 2 способа оценить наш параллельные вычисления:
...
Arrays:
 - :
  1) Нужно убеждаться в том что мы не пишем данные в одни и те де участки
...
Trees:
 ...
 + :
  ...
  2) Не надо беспокоиться о том что будут вестись записи в один и тот же участок записи.
...
Цель data - parallel планировщика: эффективно балансировать рабочую нагрузку между процессорами без знания того, сколько нужно на обработку элемента. Эта обязанность сдваивается с плеч программиста.
...
Не для все х коллекций есть их параллельные двойники, но они все равно преобразуются в ближайший похожий, так List, преобразуется в ParVector.
...
Spark очень умный и далает почти на всем оптимизацию. к примеру если мы имеем action take(10), он не будет считать всю коллекцию, а затем брать 10 элементов, он посчитает только 10 элементов, а замоем вернет их.
...
Spark app initialization: создается SparkcConxtext, SparkcConxtext коннектится к cluster manager, назначет workers, driver program отправляет код to executors, SparkcConxtext отпраляет таски executors.
...
В Spark key-value pairs храняться как Pair RDDs. Так как там представлен широкий набор возможно манипуляции над данными с ключами.
...
RangePartition - распределение данных в соответсвии с порядком ключей. К примеру, распределение меду 5 машинами Int ключей, в равном range по 5. Tuple падает в соответсивии с порядком данного ключа и set of sorted ranges of keys. Важно понмиать что ключ должен иметь какой-то порядок.
...
Linage graph - вычисления, которые происходят я RDD. Представляются в виде DAG.
...
Transformations имеют два вида зависимости:
 1) Narrow - каждый partition parent RDD используется максимум одним child partition RDD. Shuffle не нужно, и мы имеем быстрое исполнение рпеаций. (Map, filter, union, join[with partition])
...
Агрегация может быть вызвана 2-я способами:
 1) df.groupBy($"attribute1").add(sum($"attribute2"))
...
А отличие от DF, где мы имеем только Untyped transformation, в DS мы имеем как typed, так untyped tranformation.
...
Encoders - то что конвертирует JVM объект и Spark SQL внутренние представление. У нас генерируется кастомный код для стерилизации и десериализации данных. Эти данные хранятся в Tungsten. Он гораздо быстрее чем java стерилизатор, так как знает схему данных и хранит их гораздо компактнее и быстрее обрабатывает. Так же он уже оптимизирован под узкий надо данных для DS.
В классе Encoders уже есть шнеки набор Encoder для примитивов, String, case class or tule.
Просто нужно вызвать что-то на подобие: Encoders.String || Encoders.product(Person).
